...
export default function SavingsListPage() {
  ...
  const fetchAccounts = async () => {
    ...
    setError("");
    try {
      console.log('Buscando contas poupança...');
      ...
      if (!response.ok) {
        ...
        let errorMessage = "Erro ao buscar contas poupança";
  ...
  const handleDelete = async (cpf: string) => {
    if (!confirm(`Tem certeza que deseja excluir a conta poupança do CPF ${cpf}?`)) {
      return;
    }

    try {
      console.log('Excluindo conta:', cpf);
      const response = await fetch(`http://localhost:8080/api/savings/${cpf}`, {
        method: "DELETE",
        headers: { 
          "Content-Type": "application/json",
          "Accept": "application/json"
    ...
    return (
      <main className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-100 to-slate-200">
        <div className="text-center">
          <div className="text-lg text-gray-700">Carregando contas poupança...</div>
        </div>
      </main>
    );
  }
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-xl p-6 mb-6 ring-1 ring-gray-200">
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-3xl font-extrabold text-gray-800">Gerenciamento de Contas Poupança</h1>
              <p className="text-sm text-gray-600 mt-1">Visualize, edite e exclua contas poupança do sistema</p>
            </div>
            <button
        ...
        {editingAccount && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full">
              <h2 className="text-2xl font-bold text-gray-800 mb-4">Editar Conta Poupança</h2>
