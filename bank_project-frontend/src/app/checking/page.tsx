...
export default function CheckingPage() {
  ...
  const checkExistingAccount = async (userCpf: string) => {
    try {
      console.log('Verificando conta corrente para CPF:', userCpf);
      const response = await fetch(`http://localhost:8080/api/checking/${userCpf}`, {
        method: "GET",
        headers: { 
      ...
      if (response.ok) {
        ...
        console.log('Conta corrente encontrada:', data);
        setAccount(data);
        setHasAccount(true);
      } else {
  ...
  const handleCreateAccount = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        let errorMessage = "Erro ao criar conta corrente";
      ...
      console.log("Conta corrente criada:", created);
  ...
  const handleDeposit = async (e: React.FormEvent) => {
    ...
    setProcessing(true);
    try {
      const response = await fetch(`http://localhost:8080/api/checking/${user.cpf}/deposit`, {
        method: "PATCH",
        headers: { 
          "Content-Type": "application/json",
          "Accept": "application/json"
      ...
      // Recarrega a conta para obter saldo atualizado
      if (user.cpf) {
  ...
  const handleWithdraw = async (e: React.FormEvent) => {
    ...
    setProcessing(true);
    try {
      const response = await fetch(`http://localhost:8080/api/checking/${user.cpf}/withdraw`, {
        method: "PATCH",
        headers: { 
          "Content-Type": "application/json",
          "Accept": "application/json"
      ...
      <main className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-100 to-slate-200">
        <div className="w-full max-w-md bg-white/90 backdrop-blur-sm rounded-2xl shadow-xl p-8 mx-4 text-center ring-1 ring-gray-200">
          <div className="flex flex-col items-center mb-6">
            <div className="h-14 w-14 rounded-full bg-indigo-600 flex items-center justify-center text-white font-extrabold text-lg mb-4 ring-2 ring-indigo-200">
              CC
            </div>
            <h2 className="text-2xl font-semibold text-gray-800">Conta Corrente</h2>
            {hasAccount && account && (
          ...
          <form onSubmit={handleCreateAccount} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100"
                placeholder="0.00"
                required
              />
            </div>
